      ...
      <div className="w-full max-w-md z-10 space-y-6">
        ...
        <div className="flex flex-col items-center space-y-2">
          ...
          <h2 className="text-3xl font-extrabold font-sans tracking-tight text-white mt-4">
            Welcome to <span className="bg-gradient-to-r from-indigo-400 to-purple-400 bg-clip-text text-transparent">Q&A Hub</span>
          </h2>
          <p className="text-slate-400 text-sm">Role-based Questions Management System</p>
        </div>

        {/* Form Card */}
        <div className="glass-card rounded-3xl p-8 border border-slate-800 shadow-2xl relative overflow-hidden">
          <h3 className="text-lg font-semibold text-slate-200 mb-6">Sign In</h3>
          ...
          <div className="mt-6 text-center text-xs text-slate-400">
            ...
            <Link to="/register" className="text-indigo-400 hover:text-indigo-300 font-semibold">Register here</Link>
          </div>
        </div>

        {/* Demo Credentials Alert */}
        <div className="p-4 bg-indigo-950/20 border border-indigo-500/10 rounded-2xl text-xs text-indigo-300 leading-relaxed max-w-sm mx-auto text-center">
          <span className="font-bold">💡 Try Demo Logins (Password: admin123 / manager123 / employee123)</span>
          <div className="mt-1 flex flex-wrap gap-2 justify-center font-mono text-[10px] text-indigo-400">
            <span>admin@example.com</span>•<span>manager@example.com</span>•<span>employee@example.com</span>
          </div>
        </div>
      </div>
    </div>
